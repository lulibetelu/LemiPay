...
export default function Step3({ data, onPrev, onCreate, loading }: any) {
  ...
  return (
    <div className="card bg-base-200 shadow-xl rounded-2xl p-6 border border-base-300 hover:shadow-2xl transition">
      <h2 className="text-lg font-semibold text-base-content flex items-center gap-2">📝 Revisar evento</h2>
      <p className="text-xs text-neutral mt-1">Confirmá que la información esté correcta antes de crearlo.</p>
        ...
        <div className="space-y-1">
          ...
          <div className="mt-1 flex flex-wrap gap-2">
            {data.invites.length > 0 ? (
              data.invites.map((email: string, idx: number) => <InviteBadge key={idx} email={email} />)
            ) : (
      ...
      <div className="flex justify-between mt-6">
        ...
        <button className={`btn btn-primary rounded-2xl shadow-md ${loading ? "loading" : ""}`} onClick={onCreate}>
